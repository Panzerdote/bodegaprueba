...
const DB = {
    ...
    async addUnidadMedida(nombre) { const bodega = window.currentBodega || 'BODEGA'; const { data, error } = await supabaseClient.from('unidades_medida').insert([{ nombre: nombre.toUpperCase(), bodega }]).select(); if (error) { if (error.code === '23505') throw new Error('LA UNIDAD YA EXISTE'); throw error; } return data[0]; },
    ...
    async buscarInsumosNombre(busqueda) {
        ...
        if (error) throw error; const u = []; const n = new Set(); (data||[]).forEach(i => { const nl = i.nombre.toLowerCase(); if (!n.has(nl)) { n.add(nl); u.push(i); } }); return u.slice(0, 10);
    },
    ...
    async procesarIngreso(nombre, seccion, anaquel, cantidad, unidad, lote, vencimiento, comentarios) {
        try {
            ...
            if (existentes && existentes.length > 0) { const item = existentes[0]; itemId = item.id; sa = item.stock; sn = sa + cantidad; const u = { stock: sn }; if (comentarios) u.comentarios = comentarios.toUpperCase(); if (unidad) u.unidad = unidad.toUpperCase(); await this.updateInventarioItem(itemId, u); }
            else { const nuevo = await this.addInventarioItem({ nombre: nombreUpper, seccion, anaquel, stock: cantidad, unidad: unidad ? unidad.toUpperCase() : null, lote: loteUpper || null, vencimiento: vencimiento || null, comentarios: comentarios ? comentarios.toUpperCase() : null }); itemId = nuevo.id; }
