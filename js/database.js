...
const DB = {
    ...
    async addInventarioItem(item) {
        ...
        const { data, error } = await supabaseClient.from('inventario').insert([{
            ...
            codigo_barras: item.codigo_barras || null, bodega
        }]).select();
    ...
    async buscarPorCodigoBarras(codigo) {
        const bodega = window.currentBodega || 'BODEGA';
        const { data, error } = await supabaseClient.from('inventario').select('*').eq('codigo_barras', codigo).eq('bodega', bodega);
        if (error) throw error;
        return data || [];
    },

    async procesarIngreso(nombre, seccion, anaquel, cantidad, unidad, lote, vencimiento, comentarios, codigoBarras) {
        try {
            const bodega = window.currentBodega || 'BODEGA';
