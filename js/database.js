const App = {
    ...
    showIngresoModal() {
        ...
        const botonCamara = isMobile ? `<button class="btn btn-info btn-sm" onclick="App.abrirEscanerMovil('ingreso')" title="ESCANEAR">📷</button>` : '';
        UI.openModal(`<h2>NUEVO INGRESO</h2>
            ...
            <div class="form-group"><label>CÓDIGO DE BARRAS</label><div style="display:flex;gap:8px;"><input type="text" id="ing-codigo-barras" placeholder="CÓDIGO DE BARRAS" style="flex:1;">${botonCamara}</div></div>
    ...
    showSalidaModal() {
        ...
        const botonEscanear = isMobile ? `<button class="btn btn-info btn-sm" onclick="App.abrirEscanerMovil('salida')" style="margin-bottom:10px;">📷 ESCANEAR CÓDIGO DE BARRAS</button>` : '';
        UI.openModal(`<h2>NUEVA SALIDA</h2>${campoAnaquel}${botonEscanear}
    ...
    abrirEscanerMovil(tipo) {
        const isMobile = /Android|iPhone|iPad|iPod|webOS/i.test(navigator.userAgent);
        if (!isMobile) { UI.showToast('EL ESCANER POR CÁMARA SOLO FUNCIONA EN DISPOSITIVOS MÓVILES', 'warning'); return; }
        const modalContent = document.getElementById('modal-content');
        modalContent.innerHTML = `<h2>ESCANEAR CÓDIGO DE BARRAS</h2><div id="reader" style="width:100%;max-width:350px;margin:0 auto;"></div><p style="text-align:center;margin-top:10px;color:#666;font-size:12px;">APUNTE LA CÁMARA AL CÓDIGO DE BARRAS</p><div class="form-actions"><button class="btn btn-secondary" onclick="App.detenerEscaner()">CANCELAR</button></div>`;
        ...
        setTimeout(() => {
            try {
                ...
                html5QrCode.start({ facingMode: "environment" }, config, (decodedText) => {
                    html5QrCode.stop().then(() => { document.getElementById('modal').classList.remove('active'); document.body.style.overflow = ''; this.procesarCodigoBarrasMovil(decodedText, tipo); }).catch(e => console.error(e));
                }, (errorMessage) => {});
                window.html5QrCode = html5QrCode;
            } catch (e) { console.error('Error al iniciar cámara:', e); UI.showToast('ERROR AL INICIAR LA CÁMARA', 'error'); document.getElementById('modal').classList.remove('active'); document.body.style.overflow = ''; }
        }, 300);
    },

    detenerEscaner() {
        if (window.html5QrCode) { window.html5QrCode.stop().then(() => { document.getElementById('modal').classList.remove('active'); document.body.style.overflow = ''; }).catch(e => { console.error(e); document.getElementById('modal').classList.remove('active'); document.body.style.overflow = ''; }); }
        else { document.getElementById('modal').classList.remove('active'); document.body.style.overflow = ''; }
    },

    async procesarCodigoBarrasMovil(codigo, tipo) {
        try {
            const resultados = await DB.buscarPorCodigoBarras(codigo);
            if (resultados.length === 0) { UI.showToast('CÓDIGO NO ENCONTRADO: ' + codigo, 'warning'); return; }
            if (resultados.length === 1) {
                const item = resultados[0];
                if (tipo === 'ingreso') {
                    this.showIngresoModal();
                    setTimeout(() => { const ni = document.getElementById('ing-nombre'); const us = document.getElementById('ing-unidad'); const ci = document.getElementById('ing-codigo-barras'); if (ni) ni.value = item.nombre; if (us && item.unidad) { for (let i = 0; i < us.options.length; i++) { if (us.options[i].value === item.unidad) { us.selectedIndex = i; break; } } } if (ci) ci.value = codigo; }, 400);
                } else if (tipo === 'salida') {
                    this.showSalidaModal();
                    setTimeout(() => { const bi = document.getElementById('sal-busqueda'); if (bi) { bi.value = item.nombre; this.buscarInsumoSalida(); } }, 400);
                }
                UI.showToast('INSUMO ENCONTRADO: ' + item.nombre, 'success');
            } else { UI.showToast('MÚLTIPLES COINCIDENCIAS. USE LA BÚSQUEDA MANUAL.', 'warning'); }
